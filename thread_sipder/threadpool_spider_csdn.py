...
import time
from pachong.csdn_spider.my_models import Topic, Passage
from pachong.selenium_test import get_cookie
...
if __name__ == '__main__':
    ...
    task_list = []
    for i in url_part:
        data_temp.append(Topic(url=i[0], title=i[1]))
        print(i[0])
        task = excutor.submit(get_part_data, str(i[0]))
        time.sleep(1)
        task_list.append(task)
    for i in as_completed(task_list):
        print(i)
    Topic.bulk_create(data_temp)
